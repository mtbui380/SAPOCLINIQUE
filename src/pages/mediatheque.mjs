...
const figures = [
  { span: 'gc-s8', ratio: 'ar-169', name: 'tp-sutures', alt: 'Travaux pratiques de sutures sur pièce anatomique, cursus Chirurgie orale SAPO Clinique', caption: 'Fig. 1 — TP sur pièce anatomique, Cursus Chirurgie.' },
...
function figure(f) {
  return `<figure class="${esc(f.span)}">
      ${img({ name: f.name, alt: f.alt, cls: `ph ${f.ratio}`, sizes: sizesFor(f.span) })}
